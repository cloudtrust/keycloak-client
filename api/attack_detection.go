...
func (c *Client) ClearAllLoginFailures(accessToken string, realmName string) error {
	return c.delete(accessToken, url.Path(kcAttackDetectionPath), url.Param("realm", realmName))
}
...
func (c *Client) GetAttackDetectionStatus(accessToken string, realmName, userID string) (map[string]any, error) {
	...
	var err = c.get(accessToken, &resp, url.Path(kcAttackDetectionIDPath), url.Param("realm", realmName), url.Param("id", userID))
	return resp, err
}
...
func (c *Client) ClearUserLoginFailures(accessToken string, realmName, userID string) error {
	return c.delete(accessToken, url.Path(kcAttackDetectionIDPath), url.Param("realm", realmName), url.Param("id", userID))
}
