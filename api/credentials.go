...
func (c *Client) ResetPassword(accessToken string, realmName, userID string, cred keycloak.CredentialRepresentation) error {
	return c.put(accessToken, url.Path(kcResetPasswordPath), url.Param("realm", realmName), url.Param("id", userID), body.JSON(cred))
}

// LogoutAllSessions of the user.
func (c *Client) LogoutAllSessions(accessToken string, realmName, userID string) error {
	var _, err = c.post(accessToken, nil, url.Path(kcLogoutPath), url.Param("realm", realmName), url.Param("id", userID))
	return err
}
...
func (c *Client) GetCredentials(accessToken string, realmName string, userID string) ([]keycloak.CredentialRepresentation, error) {
	...
	var err = c.get(accessToken, &resp, url.Path(kcCredentialsPath), url.Param("realm", realmName), url.Param("id", userID))
	return resp, err
}
...
func (c *Client) GetCredentialTypes(accessToken string, realmName string) ([]string, error) {
	var resp = []string{}
	var err = c.get(accessToken, &resp, url.Path(kcCredentialsTypesPath), url.Param("realm", realmName))
	return resp, err
}
...
func (c *Client) UpdateLabelCredential(accessToken string, realmName string, userID string, credentialID string, label string) error {
	return c.put(accessToken, url.Path(kcLabelPath), url.Param("realm", realmName), url.Param("id", userID), url.Param("credentialID", credentialID), body.String(label), hdrAcceptJSON, hdrContentTypeTextPlain)
}
...
func (c *Client) DeleteCredential(accessToken string, realmName string, userID string, credentialID string) error {
	return c.delete(accessToken, url.Path(kcCredentialIDPath), url.Param("realm", realmName), url.Param("id", userID), url.Param("credentialID", credentialID))
}
...
func (c *Client) MoveToFirst(accessToken string, realmName string, userID string, credentialID string) error {
	_, err := c.post(accessToken, url.Path(kcMoveFirstPath), url.Param("realm", realmName), url.Param("id", userID), url.Param("credentialID", credentialID))
	return err
}
...
func (c *Client) MoveAfter(accessToken string, realmName string, userID string, credentialID string, previousCredentialID string) error {
	_, err := c.post(accessToken, url.Path(kcMoveAfterPath), url.Param("realm", realmName), url.Param("id", userID), url.Param("credentialID", credentialID), url.Param("previousCredentialID", previousCredentialID))
	return err
}
...
func (c *Client) UpdatePassword(accessToken, realm, currentPassword, newPassword, confirmPassword string) (string, error) {
	...
	return c.post(accessToken, nil, url.Path(ctAccountPasswordPath), url.Param("realm", realm), body.JSON(m))
}
...
func (c *Client) ResetPapercardFailures(accessToken, realmName, userID, credentialID string) error {
	return c.put(accessToken, url.Path(ctResetFailuresPath), url.Param("realm", realmName), url.Param("userId", userID), url.Param("credentialId", credentialID))
}
