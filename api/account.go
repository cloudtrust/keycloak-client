...
func (c *AccountClient) UpdatePassword(accessToken, realm, currentPassword, newPassword, confirmPassword string) (string, error) {
	...
	return c.client.forRealm(realm).
		post(accessToken, nil, url.Path(ctAccountPasswordPath), url.Param("realm", realm), body.JSON(m))
}
...
func (c *AccountClient) GetAccount(accessToken string, realm string) (keycloak.UserRepresentation, error) {
	...
	var err = c.client.forRealm(realm).
		get(accessToken, &resp, url.Path(ctAccountExtensionAPIPath), url.Param("realm", realm), hdrAcceptJSON)
	return resp, err
}
...
func (c *AccountClient) UpdateAccount(accessToken string, realm string, user keycloak.UserRepresentation) error {
	_, err := c.client.forRealm(realm).
		post(accessToken, nil, url.Path(ctAccountExtensionAPIPath), url.Param("realm", realm), body.JSON(user))
	return err
}
